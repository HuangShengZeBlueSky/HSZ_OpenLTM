...
class Exp_Forecast(Exp_Basic):
    ...
    def test(self, setting, test=0):
        ...
        # === 直接进行异常检测（一次完成，不依赖中间文件） ===
        # 方法/阈值可按需调整；如需通过 CLI 传参，可在 run.py 加入对应 args，这里用默认值
        anom_method = getattr(self.args, "anom_method", "mad")     # "mad" 或 "quantile"
        anom_alpha  = getattr(self.args, "anom_alpha", 3.5)        # MAD 乘子
        anom_q      = getattr(self.args, "anom_q", 0.995)          # 分位数
        only_idx    = getattr(self.args, "anom_only_idx", -1)      # -1 表示全通道
        ...
        try:
            ...
            # 仅打印样本级异常率（N 级）；如需保存标记，可在此写入 CSV
            print(f"anomaly_rate(sample-level): {anomaly_rate:.6f}  "
