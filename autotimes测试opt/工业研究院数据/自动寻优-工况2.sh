...
# 请确保 run.py 中 Exp_Factory 已经注册了 'forecast_test_all3' -> Exp_Forecast_TestAll3
task_name="forecast_test_all3" 

model_id="工况2_转速14转每秒_微调"
...
# 2. 数据路径
# =========================================================
test_root_path="/raid/hsz/OpenLTM_data_backup/datasets/中车研究院实验台数据集/工况2_转速14转每秒/test/"
# test_data_path 留空
...
# 1. 加上 -u 防止日志阻塞
# 2. 去掉了循环，只跑一次
# 3. 去掉了 --percentile 和 --vote_rate (Python 内部自己定)
python -u run.py \
  --task_name "$task_name" \
  --is_training 0 \
  --model_id "$model_id" \
